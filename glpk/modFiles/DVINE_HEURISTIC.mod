...
#printf{i in F, u in NM, v in NM} "Variable fw[%s,%d,%d]: %f\n", i, u, v, f[i,u,v];
#if fw[i,u,v] > 0 then printf{i in F, u in N, v in N} "Variable fw[%s,%s,%s]: %f\n", i,u,v,fw[i,u,v];
#printf{i in F, u in N, v in N} "Variable fw[%s,%s,%s]: %f\n", i,u,v,fw[i,u,v];
printf{i in F, u in NM, v in NM} (if f[i,u,v] > 0 then "Variable fw[%s,%s,%s]: %f\n" else ""), i, u, v, f[i,u,v];
#printf{k in K} "x[%s] = " & (if x[k] < 0 then "?" else "%g"),k, x[k];

#printf{u in NM, v in NM} "Variable thetaw[%d,%d]: %f\n", u,v, x[u,v];
printf{u in NM, v in NM} (if x[u,v] > 0 then "Variable thetaw[%d,%d]: %f\n" else ""), u,v, x[u,v];
